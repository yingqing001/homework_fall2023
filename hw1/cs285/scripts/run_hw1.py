...
def run_training_loop(params):
    ...
    for itr in range(params['n_iter']):
        ...
        if itr == 0:
            ...
            if params['do_dagger']:
                ...
                for i in range(len(paths)):
                    ac = expert_policy.get_action(paths[i]['observation'])
                    paths[i]["action"] = ac
        ...
        for _ in range(params['num_agent_train_steps_per_iter']):
          ...
          sample_ind = np.random.permutation(len(replay_buffer.obs))[:params['train_batch_size']]
          ob_batch, ac_batch = replay_buffer.obs[sample_ind], replay_buffer.acs[sample_ind]
